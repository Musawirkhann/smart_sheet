...
import { 
  X, Plus, ChevronDown, Edit, Copy, Trash2, HelpCircle, Play, 
  Palette, Type, Bold, Italic, Underline, Strikethrough,
...
const ConditionalFormattingModal = ({ 
  ...
  const updateRuleCondition = (ruleId, conditionUpdates) => {
    setRules(prev => prev.map(rule => 
      rule.id === ruleId 
  ...
  const updateRuleFormat = (ruleId, formatUpdates) => {
    setRules(prev => prev.map(rule => 
      rule.id === ruleId 
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <select
              ...
              onChange={(e) => updateRuleCondition(rule.id, { column: e.target.value })}
                  ...
                  {rules.length === 0 ? (
                    ...
                    rules.map((rule, index) => (
                      ...
                      >
                        ...
                        <AnimatePresence>
                          {expandedRule === rule.id && <RuleEditor rule={rule} />}
                        </AnimatePresence>
                      </motion.div>
                    ))
                  )}
